...
const AIRDROP_KEY  = process.env.NEXT_PUBLIC_AIRDROP_KEY ?? '';
...
async function apiFetch(path: string, options?: RequestInit) {
  const res = await fetch(`${API_BASE}/api/v1/airdrop${path}`, {
    ...options,
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${AIRDROP_KEY}`,
      ...(options?.headers ?? {}),
    },
  });
    ...
    <div className="min-h-screen py-8 px-4">
      <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-400 text-sm mt-1">
                Manage $SCORE allocations for early participants before token launch. Stored in Firestore.
              </p>
            </div>
        ...
        <p className="text-xs text-gray-600 mt-6 text-center">
          Admin-only · Stored in Firestore (airdrop_allocations) · Restricted to deployer wallet
        </p>
      </div>
    </div>
  );
